...
def swag():
    noise = PerlinNoise(octaves=3,seed=random.randint(0,9999))
    scale = 36
...
def swas():
    ...
    for z in range(13):
        for x in range(13):
            ...
            Block(position=(x,y,z),texture=sand_texture)
            Block(position=(x,y-1,z),texture=sand_texture)
            Block(position=(x,y-2,z),texture=sand_texture)
...
def creat_world():
    is_fla = random.randint(0,1)
    is_g = random.randint(0,1)
